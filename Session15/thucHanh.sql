...
BEGIN
    DECLARE v_CurrentDebt DECIMAL(15,2);
    ...
    SELECT TotalDebt INTO v_CurrentDebt
    FROM Students
    WHERE StudentID = 'SV01';

    IF v_CurrentDebt < 0 THEN
        ROLLBACK;
        SELECT 'Lỗi: Số tiền đóng vượt quá số nợ. Giao dịch đã bị hủy.' AS Message;
    ELSE
        COMMIT;
        SELECT 'Thành công: Đã cập nhật công nợ.' AS Message;
    end if;
